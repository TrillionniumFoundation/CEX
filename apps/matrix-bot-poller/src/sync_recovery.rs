...
pub(super) fn messages_url(
    ...
    url.path_segments_mut()
        .map_err(|_| anyhow!("invalid_matrix_gap_endpoint"))?
        .pop_if_empty()
...
mod tests {
    ...
    fn empty_page_with_next_cursor_must_continue() {
        ...
        pager
            .accept(&page("new", Some("middle"), vec![]), 100)
            .unwrap();
    ...
    fn only_absent_end_or_exact_stop_completes() {
        for end in [None, Some("old")] {
            let mut pager = GapPager::new("new", "old").unwrap();
            pager
                .accept(&page("new", end, vec![json!({"event_id":"e"})]), 100)
                .unwrap();
            assert!(pager.complete());
        }
    }
    ...
    fn wrong_start_and_oversized_page_are_rejected() {
        ...
        assert!(pager
            .accept(&page("a", None, vec![json!(1), json!(2)]), 1)
            .is_err());
    }
    ...
    fn url_encodes_room_and_query_without_authority_change() {
        ...
        assert_eq!(
            url.query_pairs().find(|(k, _)| k == "from").unwrap().1,
            "a&x=1"
        );
    ...
    fn endpoint_cannot_contain_credentials_or_query() {
        for base in [
            "https://user:secret@matrix.example",
    ...
    fn omitted_end_and_empty_continuation_keep_distinct_meanings() {
        let terminal: MessagePage =
            super::super::wire_response::decode(br#"{"start":"new","chunk":[]}"#).unwrap();
        let mut pager = GapPager::new("new", "old").unwrap();
        pager.accept(&terminal, 100).unwrap();
        assert!(pager.complete());

        let continuation: MessagePage =
            super::super::wire_response::decode(br#"{"start":"new","end":"middle","chunk":[]}"#)
                .unwrap();
        let mut pager = GapPager::new("new", "old").unwrap();
    ...
    fn ambiguous_and_error_pages_never_reach_pagination() {
        for bytes in [
            ...
            br#"{"start":"new","chunk":[{"type":"m.room.message","type":"m.room.member"}]}"#
                .as_slice(),
        ] {
            assert!(super::super::wire_response::decode::<MessagePage>(bytes).is_err());
        }
