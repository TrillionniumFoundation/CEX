...
pub(super) fn configured_filter(
    ...
    let filter = match filter {
        ...
        Some(raw) => {
            ...
            if trimmed.starts_with('{') {
                validate_inline(raw)?;
                let value: Value =
                    serde_json::from_str(trimmed).map_err(|_| "matrix_stream_filter_invalid")?;
                if !value.is_object() {
                    return Err("matrix_stream_filter_invalid");
                }
...
mod tests {
    ...
    fn configured_zero_is_an_id_and_only_absence_disables_filter() {
        ...
        assert!(
            configured_filter(Err(VarError::NotUnicode("not-unicode-fixture".into()))).is_err()
        );
    ...
    fn endpoint_spelling_normalizes_but_filter_bytes_remain_exact() {
        let a = describe(
            "https://MATRIX.example:443/proxy/",
            "@bot:example",
            Some(r#"{"room":{"timeline":{"limit":10}},"presence":{}}"#),
        )
        .unwrap();
        let b = describe(
            "https://matrix.example/proxy",
            "@bot:example",
            Some(r#"{ "presence": {}, "room": {"timeline": {"limit": 10}} }"#),
        )
        .unwrap();
        assert_ne!(a, b);
        assert_eq!(a["homeserver"], b["homeserver"]);
        assert_eq!(
            describe("https://MATRIX.example:443/proxy/", "@bot:example", None).unwrap(),
    ...
    fn credentials_cannot_enter_scope_or_account_endpoint() {
        for base in [
            "file:///tmp/socket",
            "https://u:secret@m.example",
            "https://m.example?token=secret",
            "https://m.example#secret",
        ] {
    ...
    fn whoami_is_bound_to_configured_account_and_proxy_prefix() {
        ...
        assert_eq!(
            url.as_str(),
            "https://m.example/proxy/_matrix/client/v3/account/whoami"
        );
        assert_eq!(
            verify_account(&json!({"user_id":"@bot:example"}), "@bot:example"),
            Ok(())
        );
        assert_eq!(
            verify_account(
                &json!({"user_id":"@bot:example","is_guest":false}),
                "@bot:example"
            ),
            Ok(())
        );
    ...
    fn no_filter_and_no_timeline_predicate_do_not_invent_restrictions() {
        ...
        assert_eq!(
            backfill_filter(Some(r#"{"room":{"rooms":["!r:e"]}}"#), "!r:e"),
            Ok(None)
        );
    }
    ...
    fn timeline_membership_predicates_are_preserved() {
        ...
        let actual = backfill_filter(Some(&source.to_string()), "!r:e")
            .unwrap()
            .unwrap();
    ...
    fn room_exclusions_cannot_be_ignored_during_backfill() {
        for raw in [
            ...
            assert_eq!(
                backfill_filter(Some(raw), "!r:e"),
                Err("matrix_gap_room_excluded_by_filter")
            );
        }
    }
    ...
    fn ids_cannot_turn_into_unfiltered_backfill() {
        for id in ["0", "opaque-id", "42"] {
            assert_eq!(
                backfill_filter(Some(id), "!r:e"),
                Err("matrix_gap_filter_id_requires_resolution")
            );
        }
    }
