...
Current cutover boundary: receipt tables are shadowed/direct-written, progression decisions prefer typed `ReceiptProgressionClass` when a receipt exists, and legacy string fields remain as endpoint/read-model compatibility fallbacks. The normalized world-home and client-feed read-model seams now expose receipt-table counts, latest receipt metadata, and typed progression-class probes so the read switch can gate receipt-backed projection cutover. Runtime `/v1/world/home` and client feed projections now also expose typed world receipt counts, progression-class groups, latest receipt metadata, and receipt feed items, aligning the JSON projection surface with the normalized receipt read-model gates.
