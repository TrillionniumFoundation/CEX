...
- `scripts/check-production-readiness.sh` 现在给出一个更严格的 pre-production verdict：串起 runtime status、native metrics smoke、operator signals、provider dead-letter 清单，以及默认要求配置的 live provider probe（由 `scripts/probe-openclaw-provider.sh` 通过 repo-local OpenClaw scope 执行）。Linux runtime 已默认启动 consumer-entry / matrix-entry 并写入 repo-local identity binding/registry/approved-revision defaults 来压实 entry governance 检查。当前 readiness 仍会因为 provider dead letters / billing quota / 未通过 live provider probe 返回 `NOT_READY`，这是刻意保守的 100% 前置门
