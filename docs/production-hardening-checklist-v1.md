...
- `scripts/check-production-readiness.sh` 现在给出一个更严格的 pre-production verdict：串起 runtime status、native metrics smoke、operator signals 与 provider dead-letter 清单；当前它会因为 provider dead letters / billing quota / entry supporting surface 未全启用而返回 `NOT_READY`，这是刻意保守的 100% 前置门
