...
- First game account client is active at `GET /account` and `GET /game/account`: it exposes player register/sign-in/profile/password-change/session-refresh/session-revoke forms and actions, optional self-serve Argon2id password registration/login through `POST /account/register` and `POST /account/login`, signed-session profile read/update through `GET/POST /account/profile`, signed-session password changes through `POST /account/password/change`, signed-session refresh through `POST /account/session/refresh`, all-device game-account session revocation through `POST /account/session/revoke`, server-verifiable status through `GET /account/session`, logout through `POST /account/logout`, and signed game-session cookies compatible with `/league/web/session`. `/app` and `/world` now surface the same account/session bridge with Account CTAs and current signed player/room state; `/world` also binds the saved account profile display name into the first-human `Who` and tactics identity surfaces through `trillionnium_game_account_player_identity_binding_v1` while gameplay state remains Rust-owned. It still does not count as public-launch account readiness without real external account/security review evidence.
...
- `GET /account` and `GET /game/account` expose the game account client for register/sign-in, signed profile management, local profile hint storage, current session status, and allowlisted `return_to` links for `/app`, `/world`, and `/league`.
...
- `GET/POST /account/profile` requires an active signed game-session cookie, and updates display name/default room only when the submitted CSRF matches the session.
...
- Account auth observability uses aggregate counters only: register successes, login successes, login failures, profile updates, password-change successes/failures, session-refresh successes, session-revoke successes, logout successes, and account-auth rate-limit hits. The observability contract explicitly forbids logging passwords, tokens, or cookie values.
- `GET /account/session` reports the current signed game-session status; `POST /account/logout` expires the server cookie.
- `/app` and `/world` render `trillionnium_game_account_surface_session_v1` cards: unauthenticated players get a same-origin Account CTA, while signed sessions show the current player and room without exposing tokens or cookie values.
- `/world` renders `trillionnium_game_account_player_identity_binding_v1` on the first-human and tactics identity surfaces. A saved `/account/profile` display name overrides the visible account identity label, while the underlying movement/combat/world mutations still use Rust-owned game state and signed-session CSRF.
- `scripts/check-trillionnium-game-account-auth.sh` gates the account client contract, endpoint advertisement, allowlisted account return links, `/app` and `/world` account-session bridge presence, no-cookie session status, password-auth posture, rate-limit configuration, aggregate auth metrics exposure, profile/password-change/session-refresh/session-revoke contracts, and public-launch boundary. Its default mode is read-only; `--mutating` is opt-in for local/password-auth smoke and verifies register/session/profile/password-change/session-refresh/session-revoke/logout, Argon2id registry storage, plaintext absence, old-password rejection, new-password login, CSRF rotation, all-device revocation, and bad-login rate limiting.
