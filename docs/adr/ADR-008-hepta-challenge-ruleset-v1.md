...
move a hard gate into a later/no-op stage, rejects duplicate or missing transitions, recomputes
the canonical JSON SHA-256, and rejects a caller-supplied hash mismatch.
...
elapsed, gameplay mutations fail closed until the Captain records `expired`.
Before grace elapses the Captain may also record `failed` or `abandoned`; at
or after the boundary the only valid manual outcome is `expired`. Terminal
outcomes are one-way and block further gameplay mutation. `submission_ready` is set only by
the existing unanimous signed finalization path. These gameplay outcomes do
not manufacture or overwrite scientific or Chain finality.
...
- A scheduler or player action must record `expired` after the grace boundary;
  the server already rejects late mutations even before that projection write.
