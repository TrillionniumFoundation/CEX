...
5. **真实 Matrix/Element 前端闭环**
   ...
   - Trillionnium League / World 命令也应闭环：`/league`、`/world`、`/world action ...`、`/craft ...`、`/season`、`/arena`、`/guild`、`/guild guild-prompt-forge`、`/raid`、`/team guild-raid-001 scout`、`/raid guild-raid-001 ...`、`/draft ...`、`/join daily-dungeon-001`、`/battle ...`、`/submit ...`、提交卡 `ledger_status=settled`、再次 `/balance` 余额变化、`/profile`、`/rewards`、`/inventory`、`/history`、`/rank`、`/loadout`
   - Web E2E 还应覆盖 `GET /world` 和 `POST /world/web/action`，确认 World Action Console、资产/事件时间线与 world marker 都能回显。
