...
- Linux job runs `bash ./scripts/gate-local-linux.sh --service-local-only --skip-db-bootstrap`; full runtime mode starts core plus entry services, seeds repo-local entry identity governance defaults under `run/linux-runtime/entry-config/`, and runs `scripts/smoke-runtime-metrics.sh` after runtime blackbox/status to assert native core/entry `/metrics` endpoints are live; `scripts/check-production-readiness.sh` and `scripts/soak-runtime.sh` are intentionally stricter production signoff tools and should stay separate from hosted CI unless live provider credentials and runtime budgets are explicitly provisioned
