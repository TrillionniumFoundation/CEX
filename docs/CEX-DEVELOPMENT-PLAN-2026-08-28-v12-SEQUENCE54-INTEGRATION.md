...
`scripts/check-sequence54-integration.py` enforces the integration-specific invariants. `scripts/check-rust-toolchain-convergence.py` performs the recursive Git-tree and blob-content toolchain scan. Existing v12 documentation, migration, exact-money, PostgreSQL, Gateway, Execution, provider, Hepta, Matrix and candidate gates remain required rather than being replaced by this document.

## 3. Governance and required checks
...
## 5. Exact-candidate qualification
...
## 6. External production blockers
