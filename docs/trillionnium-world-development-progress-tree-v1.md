...
- `[x]` Hero Tan-style exploration loop is now product-aligned for the current local scope: Rust-owned world player positions, adjacent-node movement, blocked/locked/interaction-required/room/zone transition semantics, the local playable loop, node-local NPC talk, task pickup/completion, settlement/review feedback, active task/NPC/party objective travel, node-local mentor skill practice, and lightweight combat encounter entry/return-to-map all run through the Rust world/tactics graph.
...
- [x] TW-5.3 Current `/world` visible game loop is playable through Rust-owned projection and intent forms.
  ...
  - Remaining Hero Tan-style local loop work for the current milestone is closed: node-local skill practice and lightweight combat encounter entry/return-to-map are now Rust-owned and covered by browser/web gates.
...
- Remaining next: superseded by later checkpoints; TW-6.10, TW-3.12, TW-3.13, TW-3.11, TW-2.15, TW-3.14, and TW-3.15 are now complete. Current gameplay-loop score lifts should come from real cohort/commercial evidence or multi-node/live-traffic proof, not more local-only demo loops.
...
- Remaining next at that checkpoint, later superseded by TW-2.15 completion:
  ...
  - [x] TW-3.15 lightweight combat encounter entry from exploration nodes, then return to map. Completed in the 2026-05-11 01:4x CST update below.
...
#### Update 2026-05-11 01:4x CST

- Commit: `feat: gate world combat encounters` (this commit)
- Completed TW-3.15 lightweight combat encounter entry from exploration nodes, then return to map state:
  - [x] Added `trillionnium_world_combat_encounter_loop_v1` to the Rust tactics projection and `/world` play-first exploration prompt.
  - [x] Rust derives the active encounter from the current exploration node, validates submitted `osm_game_overlay_id` and target tile, preserves legacy tactics attack behavior when no exploration overlay is submitted, and fails closed on node/overlay/target mismatch.
  - [x] `/world` renders `#world-local-combat-encounter`, `#world-local-combat-encounter-form`, and `#world-local-combat-return`, all explicitly marked as `intent_only_visualization_input`; return state is `map_ready_after_resolution` from `rust_world_combat_encounter_return_state`.
  - [x] Accepted combat routes back to `/world?tactics=1&combat=resolved#world-local-combat-encounter`; rejected combat routes back to `/world?tactics=0&combat=rejected#world-local-combat-encounter`.
  - [x] Web and browser gates assert combat encounter projection, validator metadata, return card/state, and the end-to-end local combat encounter/return loop.
  - [x] The browser map-delta conditional check now refreshes the weak ETag if a live route-runner tick changes the cursor between no-op and conditional requests, keeping the 304/server-timing gate strict without treating a real cursor change as product failure.
- Evidence:
  - `cargo fmt --all -- --check`
  - `cargo check -p consumer-entry-api`
  - `cargo test -p consumer-entry-api -- --nocapture --test-threads=1` (`136 passed`)
  ...
  - `git diff --check`
  - local-production restart/status OK
  - Web E2E: `run/league-web/web-e2e-summary-1778433774.json`, `ok=true`
  - Browser E2E: `run/league-browser/browser-e2e-summary-1778433933-1079017.json`, `ok=true`, `coverage.world_local_combat_encounter_return_loop=true`, request-failure gate green with `0` unclassified failures
  - First-human E2E: `run/first-human-session/browser-e2e-summary-1778434312-1085283.json`, `ok=true`, zero request/page/console failures
- Remaining next:
  - [ ] With TW-3.10–TW-3.15 closed, do not add more local-only demo loops for score inflation. Next product leverage should be real first-beta cohort evidence, commercial launch drill evidence, or multi-node/live-traffic latency proof.
- Constraints preserved: no Hero Tan code/text/assets/data copying, no live OSM ingestion, no MapLibre promotion, browser/web intent-only, Rust source of truth.
...
#### Update 2026-05-11 15:2x CST

- Commit: `feat: gate native region story unlock runtime` (this commit)
- Continued legally clean full content-volume alignment by moving region graph / story-arc unlocks from catalog proof into Rust-owned runtime state:
  - [x] Added `trillionnium_world_region_story_unlock_runtime_v1` for unlocked/visited regions, story arcs, nodes, mutation counts, last mutation metadata, and recent unlock events.
  - [x] `WorldTrillionniumCharacter` now carries `region_story_unlock_state` with serde-safe starter defaults for Mirror City arrival, starter region, and starter node.
  - [x] Real Rust command handlers mutate unlock state after accepted `world_map_move`, `tactics_attack`, and `tactics_complete_task` events; browser/web only submit movement/combat/task intent.
  - [x] Tactics board/home JSON exposes `region_story_unlock_runtime` plus mutation-source metadata, and `trillionnium_story_arc_catalog_json(...)` now derives arc unlock status from runtime state.
  - [x] The full-content alignment gate now marks `story_arcs` as `rust_runtime_backed` with tracked region/story runtime counts, not catalog-only coverage.
  - [x] `/world` renders `#trillionnium-region-story-unlocks` as a visualization-only panel with source-of-truth `rust_trillionnium_region_story_unlock_runtime_state`, persistence owner `world_state.world_trillionnium_characters.region_story_unlock_state`, runtime status, counts, and latest mutation evidence.
  - [x] Added normalized Postgres mirror column `world_trillionnium_characters.region_story_unlock_state` in migration `0024_add_trillionnium_region_story_unlock_runtime_column.sql`; repository shadow SQL/direct-write metadata and migration floor now include the region/story runtime column.
  - [x] Rust tests, SQL snapshot, Web E2E, Browser E2E, and first-human gates assert projection defaults, movement/combat/task mutations, persistence/write-set parity, UI markers, and the `world_region_story_unlock_runtime_loop` coverage flag.
- Evidence: `cargo fmt --all -- --check`; `cargo check -p consumer-entry-api`; `cargo test -p consumer-entry-api -- --nocapture --test-threads=1` (136 passed); `cargo clippy -p consumer-entry-api -- -D warnings`; node/bash syntax; `git diff --check`; local-production restart/status OK; SQL snapshot gate `repository_migration_floor=0024_add_trillionnium_region_story_unlock_runtime_column.sql`; Web E2E `run/league-web/web-e2e-summary-1778483103.json`; Browser E2E `run/league-browser/browser-e2e-summary-1778483132-1351145.json`, request-failure gate green with `0` unclassified failures; First-human E2E `run/first-human-session/browser-e2e-summary-1778483373-1356789.json`, zero request/page/console failures.
- Current honest scope: item/equipment, survival/time/resource-pressure, and region graph/story-arc unlocks have crossed from native catalog breadth into persisted Rust runtime mutation. Deeper combat numerics remains the next native-runtime slice.
- Constraints preserved: no Hero Tan code/text/assets/data copying, no live OSM ingestion, no MapLibre promotion, browser/web intent-only, Rust source of truth, and no local-only evidence used to lift the technical score beyond the established 9.8 ceiling.
...
- Receipt tables are now also on the typed direct-write path: `upsert_normalized_term_exchange_receipt_tables` writes `league_term_exchange_receipts` and `world_term_exchange_receipts` from `TermExchangeReceiptState` before rollback/audit snapshot export for supported world command writes and final-cutover non-world snapshot writes. Progression decisions now prefer typed `ReceiptProgressionClass` when a receipt exists, while legacy string fields stay compatible as fallbacks; normalized world-home/client-feed read models expose receipt counts, latest receipt metadata, progression-class probes, the `trillionnium_term_exchange_receipt_projection_v1` projection object, and client-feed `term_exchange_receipts` snapshots for the read-switch cutover. `/v1/world/home`, client-feed JSON projections, `/v1/client/app/:matrix_user_id` embedded feed projections, the `/app` bootstrap shell, and JSON command-response `home` payloads mirror those typed world receipt probes and, under the normalized read switch, overlay their receipt slices from the SQL read models rather than the JSON export snapshot. The runtime commerce/recovery path now also checks typed receipts first for purchase reserve/settlement/consume, refund and seller-chargeback recovery, reopen settlement, contract completion release, and health/playability readiness counts, so stale compatibility strings cannot override receipt truth. The read-switch gate now includes `normalized_client_app_feed_overlay_green` plus startup overlay validation so the embedded client-app feed cannot silently fall behind the standalone client-feed read model.
