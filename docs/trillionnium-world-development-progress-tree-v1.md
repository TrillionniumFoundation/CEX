...
- Receipt tables are now also on the typed direct-write path: `upsert_normalized_term_exchange_receipt_tables` writes `league_term_exchange_receipts` and `world_term_exchange_receipts` from `TermExchangeReceiptState` before rollback/audit snapshot export for supported world command writes and final-cutover non-world snapshot writes. Progression decisions now prefer typed `ReceiptProgressionClass` when a receipt exists, while legacy string fields stay compatible as fallbacks; normalized world-home/client-feed read models expose receipt counts, latest receipt metadata, progression-class probes, the `trillionnium_term_exchange_receipt_projection_v1` projection object, and client-feed `term_exchange_receipts` snapshots for the read-switch cutover. `/v1/world/home` and client-feed JSON projections mirror those typed world receipt probes and surface recent receipt feed items.
