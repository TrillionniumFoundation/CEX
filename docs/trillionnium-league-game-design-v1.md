...
Current first slice exposes this as `GET /world` plus Matrix `/world`, `/world action <自由行动>`, `/assets`, `/upgrade <asset-id|latest> <升级内容>`, `/contract <委托内容>`, `/complete <contract-id> <交付内容>`, and `/craft <建造内容>`. The web shell shows zones, locations, Agent residents/NPCs, player assets, upgrade form, World Contracts, completion form, and the world event timeline.
