...
Sequence-51 closure contract: `docs/architecture/external-agent-runtime-boundary-sequence-51.md`

This addendum deepens the parent plan without changing its exact-money invariants, migration head, rollout posture or external approval boundary. The accepted ADR controls the product/runtime boundary: parent-plan provider-dispatch requirements are retained historical command/evidence and reconciliation requirements, not authority for CEX to execute participating Agents. The parent plan and addendum otherwise form the active repository implementation plan.
...
- `scripts/check-external-production-evidence-contract.py --self-test` exercises the inherited binding cases plus Sequence-50 mixed-output, exact-snapshot, candidate-artifact-isolation and repository-ownership cases;
...
- the default library/router does not expose an executable Ollama/OpenClaw adapter path;
...
- no authoritative workflow, deployment manifest or production/example configuration activates that feature or opt-in;
- the default Execution integration target proves no network/process provider execution and no prompt disclosure;
...
- `scripts/check-external-agent-runtime-boundary.py` validates the decision, source, feature, routing, capability, documentation, activation-surface and candidate-sequence facts;
...
Block M can close the repository contradiction only. It cannot prove an external Agent deployment, provider legal approval, production credential custody, real reconciliation, downstream World/Game binding or final release authorization.
...
4. the external-Agent-only runtime and strict capability registry boundary;
...
7. report actual branch/ruleset enforcement without inference;
...
Repository automation cannot close representative-volume disaster recovery; real deployment/cutover/rollback; real external Agent/provider outcomes; credential custody; sustained production-like soak; independent security, operations and financial-control review; legal/commercial/provider approval; downstream immutable World/Game revision binding; protected-branch administration; or final human go/no-go.
...
- ADR-004 and Block M are enforced by source and checker, with no local inference in the default build;
...
- no placeholder, temporary convergence workflow, broad warning allowance, fabricated external evidence, waiver or fabricated approval is used;
