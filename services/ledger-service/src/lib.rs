...
pub fn build_router(state: AppState) -> Router {
    Router::new()
        ...
        .route(
            "/v2/account-projections",
            post(account_control::capture_projection),
        )
        .route(
            "/v2/account-projections/status/:org_id",
            get(account_control::projection_status),
        )
        .route(
            "/v2/account-projections/policy",
            post(account_control::set_read_policy),
        )
        .route(
