#[allow(
    dead_code,
    reason = "the shared runtime guard serves five binaries while execution-service constructs only the execution variant and consumes only its startup fields"
)]
