    ...
    r#"      const createRealWorldMapAdapter = () => ({
      ...
      const mapText = (value) => {
        ...
        const replacements = [
          ...
          ['route_task', 'route task / 路线任务'], ['avatar_task_route', 'avatar task route / 角色任务路线'], ['avatar_route_runner', 'avatar route runner / 跑图角色'], ['Task routes', 'Task routes / 任务路线'], ['Avatar Task Routes', 'Avatar Task Routes / 角色任务路线'], ['Avatar Movement', 'Avatar Movement / 角色跑图'], ['Moving avatars', 'Moving avatars / 动态角色'], ['Trace route / 追踪路线', 'Trace route / 追踪路线'], ['Follow runner / 跟随角色', 'Follow runner / 跟随角色'], ['Complete checkpoint / 完成检查点', 'Complete checkpoint / 完成检查点'], ['Reward checkpoint / 奖励检查点', 'Reward checkpoint / 奖励检查点'], ['Prepare reward claim / 准备领奖', 'Prepare reward claim / 准备领奖'], ['Claim rating/reward / 领取评级奖励', 'Claim rating/reward / 领取评级奖励'], ['Open next route / 开启下一条路线', 'Open next route / 开启下一条路线'], ['Preview next route / 预览下一路线', 'Preview next route / 预览下一路线'], ['Next route / 下一路线', 'Next route / 下一路线'], ['Next route ready / 下一路线就绪', 'Next route ready / 下一路线就绪'], ['Next route preview / 下一路线预览', 'Next route preview / 下一路线预览'], ['Checkpoint history / 检查点历史', 'Checkpoint history / 检查点历史'], ['Reward history / 奖励历史', 'Reward history / 奖励历史'], ['Agent party / Agent 小队', 'Agent party / Agent 小队'], ['Party state / 小队状态', 'Party state / 小队状态'], ['party members / 个小队成员', 'party members / 个小队成员'], ['Agent handoff / Agent 交接', 'Agent handoff / Agent 交接'], ['Scout route / 侦察路线', 'Scout route / 侦察路线'], ['Build deliverable / 构建交付', 'Build deliverable / 构建交付'], ['Audit risk / 审计风险', 'Audit risk / 审计风险'], ['Close reward / 结算奖励', 'Close reward / 结算奖励'], ['Oracle Scout / 预判侦察', 'Oracle Scout / 预判侦察'], ['Forge Builder / 交付锻造', 'Forge Builder / 交付锻造'], ['Mirror Auditor / 镜像审计', 'Mirror Auditor / 镜像审计'], ['Courier Closer / 结算信使', 'Courier Closer / 结算信使'], ['history steps / 个历史节点', 'history steps / 个历史节点'], ['Ready to complete / 可完成', 'Ready to complete / 可完成'], ['Reward checkpoint locked / 奖励检查点未解锁', 'Reward checkpoint locked / 奖励检查点未解锁'], ['Approaching checkpoint / 接近检查点', 'Approaching checkpoint / 接近检查点'], ['Submit evidence → rating/reward / 提交证据 → 评级奖励', 'Submit evidence → rating/reward / 提交证据 → 评级奖励'], ['animated path / 动态路径', 'animated path / 动态路径'], ['looping_avatar_task_run', 'looping avatar task run / 循环任务跑图'], ['en_route_to_task_reward', 'en route to task reward / 正在跑向任务奖励'], ['ready_to_complete', 'ready to complete / 可完成'], ['route_started', 'route started / 路线开始'], ['evidence_checkpoint', 'evidence checkpoint / 证据检查点'], ['reward_settlement', 'rating/reward settlement / 评级奖励结算'], ['next_route_handoff', 'next route handoff / 下一路线交接'], ['next_route_ready_after_reward_claim', 'next route ready after reward claim / 领奖后开启下一路线'], ['next_route_preview_locked_until_reward_claim', 'next route preview locked until reward claim / 领奖前预览下一路线'], ['claimable_next', 'claimable next / 下一步可领取'], ['locked_until_checkpoint', 'locked until checkpoint / 检查点前锁定'], ['locked_until_evidence_checkpoint', 'locked until evidence checkpoint / 证据检查点前锁定'], ['claimable_after_evidence', 'claimable after evidence / 证据完成后可领取'], ['contract_capture', 'contract capture / 契约登记'], ['work_order', 'quest commission / 冒险委托'], ['delivery', 'result submit / 成果提交'], ['acceptance', 'rating pass / 评级'], ['rejection', 'revision / 返工'], ['reopen', 'reopen / 重开'], ['cancellation', 'cancel / 放弃'], ['live_event', 'live event / 实时事件'],
...
pub(super) fn real_world_map_card_focus_helpers_js() -> &'static str {
    r#"      const mapFocusButtonAttrs = (attrs) => Object.entries(attrs || {})
      ...
      const mapViewportCardModel = (item, kind, style = 'app') => {
        ...
        if (kind === 'cluster') {
          return {
            className: worldStyle ? 'mini cluster' : 'module',
            title: mapText((source.marker_count ?? 0) + ' places cluster / 地点聚合'),
            meta: `${mapText(source.primary_node_kind || 'mixed / 混合')} · ${source.distance_km ?? 0} km · ${mapText(source.render_policy || 'aggregate_marker_cluster')}`,
            code: source.cluster_id || source.tile_id || 'marker_cluster',
            focusHtml: mapRegionFocusButton({ center: source.center || {}, zoom_max: Math.min(19, Number(source.z || 14) + 1), name: source.cluster_id || 'Marker cluster' }, 'Open cluster / 展开聚合'),
          };
        }
        if (kind === 'event') {
          return {
...
pub(super) fn real_world_map_render_cards_js(style: RealWorldMapShellCardStyle) -> String {
    ...
    format!(
        r#"      const rustLiveTaskUiFragmentForFocus = (viewport, focus) => {{
        const fragments = (viewport || {{}}).rust_owned_live_task_ui_fragments || {{}};
        const taskId = String((focus || {{}}).taskId || '').trim();
        const locationId = String((focus || {{}}).locationId || '').trim();
        const nodeId = String((focus || {{}}).nodeId || '').trim();
        const eventId = String((focus || {{}}).eventId || '').trim();
        return (eventId && fragments.by_event_id && fragments.by_event_id[eventId])
          || (taskId && fragments.by_task_id && fragments.by_task_id[taskId])
          || (locationId && fragments.by_location_id && fragments.by_location_id[locationId])
          || (nodeId && fragments.by_node_id && fragments.by_node_id[nodeId])
          || fragments.default
          || null;
      }};
      const renderRustLiveTaskCards = (liveEventNode, taskRouteNode, viewport, focus) => {{
        const fragment = rustLiveTaskUiFragmentForFocus(viewport, focus);
        let rendered = false;
        if (liveEventNode && liveEventNode.dataset && liveEventNode.dataset.rustLiveTaskUiContract) {{
          const expectedContract = liveEventNode.dataset.rustLiveTaskUiContract || 'trillionnium_world_rust_live_task_ui_fragments_v1';
          if (fragment && String(fragment.contract_version || '') === expectedContract && typeof fragment.live_events_html === 'string') {{
            liveEventNode.innerHTML = fragment.live_events_html;
            liveEventNode.dataset.renderOwner = 'rust_world_ui_renderer';
            liveEventNode.dataset.browserUiOwner = 'input_only_focus_bridge';
            rendered = true;
          }}
        }}
        if (taskRouteNode && taskRouteNode.dataset && taskRouteNode.dataset.rustLiveTaskUiContract) {{
          const expectedContract = taskRouteNode.dataset.rustLiveTaskUiContract || 'trillionnium_world_rust_live_task_ui_fragments_v1';
          if (fragment && String(fragment.contract_version || '') === expectedContract && typeof fragment.task_routes_html === 'string') {{
            taskRouteNode.innerHTML = fragment.task_routes_html;
            taskRouteNode.dataset.renderOwner = 'rust_world_ui_renderer';
            taskRouteNode.dataset.browserUiOwner = 'input_only_focus_bridge';
            rendered = true;
          }}
        }}
        return rendered;
      }};
      const rustRouteRunnerUiFragmentForFocus = (viewport, focus) => {{
...
pub(super) fn real_world_map_route_flow_buttons_js() -> &'static str {
    r#"      const buildRouteActionKey = (action) => {
      ...
      const buildRouteRunnerRewardClaimAction = (runner) => {
        const source = runner || {};
        const checkpoint = source.reward_checkpoint || {};
        const claim = checkpoint.reward_claim_action || {};
        return buildSimpleRouteTargetAction({
          label: claim.label || source.reward_claim_label || checkpoint.reward_claim_label || 'Prepare reward claim / 准备领奖',
          panelId: claim.panel_id || claim.panelId || routeActionPanelId(),
          textareaId: claim.textarea_id || claim.textareaId || routeActionTextareaId(),
          locationId: source.latest_location_id || '',
          targetNodeId: claim.node_id || source.to_node_id || checkpoint.node_id || '',
          taskId: claim.task_id || claim.taskId || source.task_id || '',
          status: claim.status || source.reward_claim_status || source.completion_status || '',
          body: claim.body || source.reward_claim_action_body || source.completion_action_body || source.completion_command || '',
        });
      };
      const routeRunnerRewardClaimButtonHtml = (runner, className = 'trillionnium-route-flow-action trillionnium-reward-claim-action') => {
        return routeFlowActionButtonHtml(buildRouteRunnerRewardClaimAction(runner), className);
      };
      const buildRouteRunnerNextRouteAction = (runner) => {
        const source = runner || {};
        const checkpoint = source.reward_checkpoint || {};
        const nextRoute = checkpoint.next_route_action || {};
        return buildSimpleRouteTargetAction({
          label: nextRoute.label || source.next_route_label || 'Open next route / 开启下一条路线',
          panelId: nextRoute.panel_id || nextRoute.panelId || routeActionPanelId(),
          textareaId: nextRoute.textarea_id || nextRoute.textareaId || routeActionTextareaId(),
          locationId: source.latest_location_id || '',
          targetNodeId: nextRoute.node_id || source.to_node_id || checkpoint.node_id || '',
          taskId: nextRoute.task_id || nextRoute.taskId || source.task_id || '',
          status: nextRoute.status || source.next_route_status || source.reward_claim_status || '',
          body: nextRoute.body || source.next_route_action_body || source.reward_claim_action_body || source.reward_loop || '',
        });
      };
      const routeRunnerNextRouteButtonHtml = (runner, className = 'trillionnium-route-flow-action trillionnium-next-route-action') => {
        return routeFlowActionButtonHtml(buildRouteRunnerNextRouteAction(runner), className);
      };
      ...
      const buildAgentPartyAction = (member, source) => {
        ...
        const handoff = partyMember.handoff_action || {};
        return buildSimpleRouteTargetAction({
          label: handoff.label || partyMember.action_label || partyMember.display_name || 'Agent handoff / Agent 交接',
          panelId: handoff.panel_id || handoff.panelId || routeActionPanelId(),
          textareaId: handoff.textarea_id || handoff.textareaId || routeActionTextareaId(),
          locationId: routeSource.latest_location_id || routeSource.location_id || '',
          targetNodeId: routeSource.to_node_id || routeSource.node_id || '',
          taskId: handoff.task_id || handoff.taskId || routeSource.task_id || '',
          status: handoff.status || partyMember.state || '',
          body: handoff.body || partyMember.action_body || partyMember.responsibility || '',
        });
      };
      const agentPartyActionButtonsHtml = (source, className = 'trillionnium-route-flow-action trillionnium-app-route-flow-action trillionnium-agent-party-action') => {
        const party = Array.isArray((source || {}).agent_party) ? source.agent_party : [];
        return party.slice(0, 4).map((member) => routeFlowActionButtonHtml(buildAgentPartyAction(member, source), className)).join(' ');
      };
      const agentPartyChipsHtml = (source, className = 'trillionnium-route-flow-action trillionnium-app-route-flow-action trillionnium-agent-party-action') => {
        const party = Array.isArray((source || {}).agent_party) ? source.agent_party : [];
        const chips = [];
        ...
        if ((source || {}).agent_party_action_summary) chips.push(`<span class="hud-chip">${escapeHtml(mapText(source.agent_party_action_summary))}</span>`);
        const actions = agentPartyActionButtonsHtml(source, className);
        if (actions) chips.push(actions);
        return chips.join(' ');
      };
...
pub(super) fn real_world_map_viewport_hydration_js() -> &'static str {
    r#"      const viewportApi = (engine.viewport_api || {});
      ...
      let lastFocusActionRumAt = 0;
      const installMapRuntimeRumObservers = (() => {
        try {
          if (typeof PerformanceObserver !== 'undefined') {
            const longTaskObserver = new PerformanceObserver((list) => {
              ...
              entries.forEach((entry) => {
                ...
                if (duration > 0) window.trillionniumMapLastLongTaskMs = Math.max(Number(window.trillionniumMapLastLongTaskMs || 0), duration);
              });
            });
            longTaskObserver.observe({ entryTypes: ['longtask'] });
            window.trillionniumMapLongTaskObserverInstalled = true;
          }
        } catch (_error) {
          window.trillionniumMapLongTaskObserverInstalled = false;
        }
        try {
          document.addEventListener('click', (event) => {
            const targetNode = event.target && event.target.closest ? event.target.closest('.trillionnium-map-focus, .trillionnium-route-flow-action, .trillionnium-app-route-flow-action, .trillionnium-reward-claim-action, .trillionnium-next-route-action') : null;
            if (!targetNode) return;
            const now = mapNowMs ? mapNowMs() : ((typeof performance !== 'undefined' && performance.now) ? performance.now() : Date.now());
            if (now - lastFocusActionRumAt < 120) return;
            lastFocusActionRumAt = now;
            window.trillionniumMapLastFocusActionAt = now;
            postMapRumSample({ sample_kind: 'focus_to_action_rail', focus_to_action_rail_ms: 1 });
          }, { capture: true, passive: true });
          window.trillionniumMapFocusActionRumInstalled = true;
        } catch (_error) {
          window.trillionniumMapFocusActionRumInstalled = false;
        }
        return true;
      })();
      ...
      const applyViewportDelta = (delta) => {
        ...
        if (delta.counts) {
          ...
          viewport.marker_cluster_count = delta.counts.marker_cluster_count ?? viewport.marker_cluster_count;
      ...
      let viewportSecondaryRenderHandle = null;
      let viewportRenderGeneration = 0;
      const normalizeViewportChangedGroups = (groups) => {
        if (!Array.isArray(groups)) return null;
        return new Set(groups.map((group) => String(group || '').trim()).filter(Boolean));
      };
      const viewportShouldRenderGroup = (changedGroups, ...groupIds) => {
        if (!changedGroups || changedGroups.size === 0) return false;
        return groupIds.some((groupId) => changedGroups.has(groupId));
      };
      const renderViewportCardsAndOverlays = (viewport, changedGroups = null) => {
        ...
        if (shouldRender('avatar_task_routes', 'rust_owned_live_task_ui_fragments')) {
          safeMapRender('task_route_cards', () => renderRustLiveTaskCards(null, taskRouteTarget, viewport, lastSelection) || renderCards(taskRouteTarget, filterAvatarTaskRoutes(viewport.avatar_task_routes || [], lastSelection), 'taskRoute'));
        }
      ...
      const scheduleViewportSecondaryRender = (viewport, changedGroups = null) => {
        const generation = ++viewportRenderGeneration;
        if (viewportSecondaryRenderHandle && typeof cancelIdleCallback === 'function') {
          try { cancelIdleCallback(viewportSecondaryRenderHandle); } catch (_error) {}
        } else if (viewportSecondaryRenderHandle) {
          window.clearTimeout(viewportSecondaryRenderHandle);
        }
        const run = () => {
          if (generation !== viewportRenderGeneration) return;
          renderViewportCardsAndOverlays(viewport, changedGroups);
          refreshMapRuntimeDiagnostics();
        };
        if (typeof requestIdleCallback === 'function') {
          viewportSecondaryRenderHandle = requestIdleCallback(run, { timeout: 120 });
        } else {
          viewportSecondaryRenderHandle = window.setTimeout(run, 0);
        }
      };
      const applyViewportSnapshot = (viewport, mapCenter, zoom, options = {}) => {
        ...
        if (cameraSummary) {
          const clusterText = (viewport.marker_cluster_count || 0) ? (' · ' + (viewport.marker_cluster_count || 0) + ' clusters') : '';
          cameraSummary.textContent = routePhrase('Camera ' + mapCenter.lat.toFixed(4) + ', ' + mapCenter.lng.toFixed(4) + ' · zoom ' + zoom + ' · ' + mapText(viewport.lod_mode || 'street_nodes') + ' · ' + (viewport.marker_count || 0) + ' visible places' + clusterText + ' · ' + mapText(((viewport.player_density || {}).mode) || 'dense') + ' density · ' + (viewport.live_event_count || 0) + ' live events', '镜头 ' + mapCenter.lat.toFixed(4) + ', ' + mapCenter.lng.toFixed(4) + ' · 缩放 ' + zoom + ' · ' + mapText(viewport.lod_mode || 'street_nodes') + ' · ' + (viewport.marker_count || 0) + ' 个可见地点' + ((viewport.marker_cluster_count || 0) ? (' · ' + (viewport.marker_cluster_count || 0) + ' 个聚合') : '') + ' · ' + mapText(((viewport.player_density || {}).mode) || 'dense') + ' 密度 · ' + (viewport.live_event_count || 0) + ' 个实时事件');
        }
        if (typeof renderRouteRunnerHandoffSummary === 'function' && (!changedGroups || viewportShouldRenderGroup(changedGroups, 'route_runner_handoff', 'avatar_route_runners', 'avatar_task_routes'))) {
        ...
        window.trillionniumMapViewportRefreshMode = skipCardRender ? 'core_noop_fast_path' : (changedGroups ? 'changed_group_deferred_render' : 'full_deferred_render');
        ...
        scheduleViewportSecondaryRender(viewport, changedGroups);
        return viewport;
      };
      let viewportFetchGeneration = 0;
      let viewportFetchAbortController = null;
      const mapNowMs = () => (typeof performance !== 'undefined' && performance.now) ? performance.now() : Date.now();
      const mapElapsedMs = (startedAt) => Math.max(0, Math.round(mapNowMs() - startedAt));
      const postFirstMapInteractive = (sampleKind = 'first_map_interactive') => {
        if (mapRumFirstInteractiveSent) return;
        mapRumFirstInteractiveSent = true;
        postMapRumSample({ sample_kind: sampleKind, first_map_interactive_ms: Math.max(0, Math.round(mapNowMs() - mapRuntimeStartedAt)) });
      };
      const fetchViewportSnapshot = async () => {
        const fetchGeneration = ++viewportFetchGeneration;
        if (viewportFetchAbortController && typeof viewportFetchAbortController.abort === 'function') {
          try { viewportFetchAbortController.abort(); } catch (_error) {}
        }
        viewportFetchAbortController = (typeof AbortController !== 'undefined') ? new AbortController() : null;
        const fetchSignal = viewportFetchAbortController ? viewportFetchAbortController.signal : undefined;
        ...
        if (lastViewportCursor) {
          try {
            ...
            const deltaResponse = await fetch(buildViewportDeltaUrl(mapCenter, zoom, lastViewportCursor), { credentials: 'same-origin', cache: 'no-cache', signal: fetchSignal, headers: lastViewportCursor ? { 'if-none-match': conditionalEtag, 'x-trillionnium-map-if-none-match': conditionalEtag } : {} });
            if (fetchGeneration !== viewportFetchGeneration) return lastViewport;
            if (deltaResponse.status === 304 && lastViewport) {
              const coreStartedAt = mapNowMs();
              applyViewportSnapshot(lastViewport, mapCenter, zoom, { skipCardRender: true, changedGroups: [] });
              postFirstMapInteractive('first_map_interactive_delta_304_ready');
              const elapsed = mapElapsedMs(coreStartedAt);
              postMapRumSample({ sample_kind: 'delta_not_modified_304_fast_path', viewport_refresh_ms: elapsed });
              return lastViewport;
            }
            if (deltaResponse.ok) {
              ...
              if (patchedViewport) {
                const changedGroups = (((delta.entity_delta || {}).changed_groups) || []);
                const isNoop = delta.changed === false || changedGroups.length === 0;
                const coreStartedAt = mapNowMs();
                applyViewportSnapshot(patchedViewport, mapCenter, zoom, { changedGroups, skipCardRender: isNoop });
                postFirstMapInteractive('first_map_interactive_delta_ready');
                const elapsed = mapElapsedMs(coreStartedAt);
                postMapRumSample({ sample_kind: isNoop ? 'delta_noop_fast_path' : 'delta_viewport_refresh_deferred_render', viewport_refresh_ms: elapsed });
                return patchedViewport;
              }
            }
          } catch (error) {
            if (error && error.name === 'AbortError') {
              window.trillionniumMapViewportAbortCount = (window.trillionniumMapViewportAbortCount || 0) + 1;
              return lastViewport;
            }
            postMapRumSample({ sample_kind: 'delta_fetch_error_weak_network' });
          }
        }
        try {
          const response = await fetch(buildViewportUrl(mapCenter, zoom), { credentials: 'same-origin', signal: fetchSignal });
          if (fetchGeneration !== viewportFetchGeneration) return lastViewport;
          if (!response.ok) throw new Error('snapshot viewport failed: ' + response.status);
          const viewport = await response.json();
          if (fetchGeneration !== viewportFetchGeneration) return lastViewport;
          const coreStartedAt = mapNowMs();
          applyViewportSnapshot(viewport, mapCenter, zoom, { deferCardRender: true });
          postFirstMapInteractive('first_map_interactive_snapshot_ready');
          const elapsed = mapElapsedMs(coreStartedAt);
          postMapRumSample({ sample_kind: 'snapshot_viewport_refresh_deferred_render', viewport_refresh_ms: elapsed });
          return viewport;
        } catch (error) {
          if (error && error.name === 'AbortError') {
            window.trillionniumMapViewportAbortCount = (window.trillionniumMapViewportAbortCount || 0) + 1;
          ...
          if (cachedViewport) {
            const coreStartedAt = mapNowMs();
            applyViewportSnapshot(cachedViewport, mapCenter, zoom, { skipCardRender: true, changedGroups: [] });
            postFirstMapInteractive('first_map_interactive_cached_ready');
            const elapsed = mapElapsedMs(coreStartedAt);
      ...
      const refreshMapRuntimeDiagnostics = () => {
        ...
        const diagnostics = {
          ...
          viewport_refresh_mode: window.trillionniumMapViewportRefreshMode || 'not_started',
          viewport_abort_count: window.trillionniumMapViewportAbortCount || 0,
          refresh_hook_available: typeof fetchViewportSnapshot === 'function',
          changed_group_deferred_render_available: typeof scheduleViewportSecondaryRender === 'function',
          rum_focus_action_observer_installed: window.trillionniumMapFocusActionRumInstalled === true,
          rum_long_task_observer_installed: window.trillionniumMapLongTaskObserverInstalled !== false,
          shadow_probe_exported: Boolean(window.trillionniumMapLibreShadowProbe),
          marker_cluster_count: Number((lastViewport || {}).marker_cluster_count || 0),
          rum_last_sample_excludes_lat_lng: window.trillionniumMapRumLastSample?.precise_lat_lng_excluded === true,
          readiness_checks: ['refresh_hook_available', 'viewport_abort_previous_request_available', 'changed_group_deferred_render_available', 'rum_focus_action_observer_installed', 'rum_long_task_observer_installed', 'slim_cached_snapshot_available_after_first_viewport', 'rum_last_sample_excludes_lat_lng', 'shadow_probe_exported', 'marker_clusters_visible'],
        };
      ...
      if (typeof requestAnimationFrame === 'function') {
        requestAnimationFrame(() => postFirstMapInteractive('first_map_interactive_runtime_ready'));
      } else {
        window.setTimeout(() => postFirstMapInteractive('first_map_interactive_runtime_ready'), 0);
      }
"#
}
