      ...
      const mapViewportCardModel = (item, kind, style = 'app') => {
        ...
        if (kind === 'routeRunner') {
          ...
          return {
            ...
            focusHtml: `${mapAvatarTaskRouteFocusButton(source, 'Follow runner / 跟随角色')} ${completionButton} ${rewardClaimButton} <span class="hud-chip">${escapeHtml(mapText(source.progress_label || `${progress}% route progress / ${progress}% 路线进度`))}</span> <span class="hud-chip">${escapeHtml(etaLabel)}</span> <span class="hud-chip">${traceCount} ${escapeHtml(mapText('trace points / 个追踪点'))}</span> <span class="hud-chip">${escapeHtml(checkpointLabel)}</span> <span class="hud-chip">${escapeHtml(mapText(source.completion_label || 'Complete checkpoint / 完成检查点'))}</span> ${historyChips} ${partyChips}`,
          };
        }
        return {
