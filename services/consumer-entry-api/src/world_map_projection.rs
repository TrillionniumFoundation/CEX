...
fn route_runner_epoch_from_cursor(cursor: Option<&str>) -> Option<i64> {
    let cursor = cursor?;
    let base = cursor.split_once(";gv=").map_or(cursor, |(base, _)| base);
    base.split(';')
...
fn route_runner_delta_epoch_for_now(cursor: Option<&str>, now_epoch: i64) -> i64 {
    ...
    let previous_is_immediately_adjacent = current_tick.saturating_sub(previous_tick)
...
impl<'a> WorldHomeProjectionContext<'a> {
    ...
    fn map_runtime_fields(&self) -> Map<String, Value> {
        ...
        let current_map_node = self.current_map_node(&map_nodes_for_engine);
        let mut fields = Map::new();
        fields.insert(
            "real_world_map_engine".to_string(),
            real_world_map_engine_json(&map_nodes_for_engine, current_map_node),
        );
        fields.insert(
            "openstreetmap_geodata".to_string(),
            openstreetmap_geodata_v1_json(&map_nodes_for_engine, current_map_node),
        );
        fields
    }
...
pub(super) fn real_world_node_marker_json(node: &WorldMapNode) -> Value {
    ...
    json!({
        ...
        "openstreetmap_geodata_contract": OPENSTREETMAP_GEODATA_CONTRACT_VERSION,
        "x": node.x,
        "y": node.y,
...
fn world_map_viewport_json_at_epoch(
    ...
    json!({
        ...
        "gameplay_accessibility_i18n": trillionnium_world_map_gameplay_accessibility_i18n_json(),
        ...
        "viewport_contract": {
            ...
            "supports_real_user_rum_matrix": true,
            "real_user_rum_matrix_contract_version": TRILLIONNIUM_WORLD_MAP_REAL_USER_RUM_MATRIX_CONTRACT_VERSION,
            "supports_weak_network_resilience": true,
            "weak_network_contract_version": TRILLIONNIUM_WORLD_MAP_WEAK_NETWORK_CONTRACT_VERSION,
            "supports_offline_action_queue": true,
            "offline_action_queue_contract_version": TRILLIONNIUM_WORLD_MAP_OFFLINE_ACTION_QUEUE_CONTRACT_VERSION,
            "supports_location_privacy": true,
            "location_privacy_contract_version": TRILLIONNIUM_WORLD_MAP_LOCATION_PRIVACY_CONTRACT_VERSION,
            "supports_density_scalability": true,
            "density_scalability_contract_version": TRILLIONNIUM_WORLD_MAP_DENSITY_SCALABILITY_CONTRACT_VERSION,
            "supports_gameplay_accessibility_i18n": true,
            "gameplay_accessibility_contract_version": TRILLIONNIUM_WORLD_MAP_GAMEPLAY_ACCESSIBILITY_CONTRACT_VERSION,
...
mod route_runner_cursor_tests {
    ...
    fn cursor_epoch_parser_is_bounded_to_the_runner_component() {
        ...
        assert_eq!(
            route_runner_epoch_from_cursor(Some("region=core;gv=r=105")),
            None
        );
        assert_eq!(
            route_runner_epoch_from_cursor(Some("region=core;r=-1;gv=x:y")),
            None
        );
        assert_eq!(
            route_runner_epoch_from_cursor(Some("region=core;r=bad;gv=x:y")),
            None
        );
    }
