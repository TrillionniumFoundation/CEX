...
const replayControllerStart = source.indexOf("function createTimelineReplayController(");
const replayBeginStart = source.indexOf("const begin = () => {", replayControllerStart);
const replayStopStart = source.indexOf("const stop = () => {", replayBeginStart);
assert.ok(replayControllerStart >= 0 && replayBeginStart > replayControllerStart);
assert.ok(replayStopStart > replayBeginStart);
const replayBegin = source.slice(replayBeginStart, replayStopStart);
const replaySignalStart = replayBegin.indexOf('recordProductEvent("replay_started", {');
const replaySignalEnd = replayBegin.indexOf("});", replaySignalStart);
assert.ok(replaySignalStart >= 0 && replaySignalEnd > replaySignalStart);
const replaySignal = replayBegin.slice(replaySignalStart, replaySignalEnd);
assert.ok(replaySignal.includes("paperId: card.dataset.paperId || null"));
assert.equal(replaySignal.includes("authority"), false);
assert.equal(replaySignal.includes("hash"), false);
assert.equal(replaySignal.includes("digest"), false);
assert.equal(replaySignal.includes("signature"), false);
assert.equal(replaySignal.includes("uuid("), false);
assert.ok(replayBegin.includes("}).catch(() => {});"));
assert.ok(replaySignalStart < replayBegin.indexOf("running = true"));
assert.ok(htmlSource.includes("best-effort identifier-only replay_started signal"));
