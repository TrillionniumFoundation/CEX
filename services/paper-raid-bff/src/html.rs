...
pub fn login_page() -> Response {
    let body = r#"<section class="hero"><span class="eyebrow">PAPER RAID · ALPHA</span><h1>Research together.</h1><p>Use the closed-Alpha access key issued to you by the operator. In invite mode its first successful use atomically activates your account; fixed-Alpha keys continue to work unchanged. The full flow separates 3 Author Raid identities from at least 4 independent evaluation, review, and reproduction identities. The key is cleared immediately and is never written to browser storage.</p></section><section class="panel narrow"><form id="login-form"><label>Closed-Alpha access key / <span lang="zh-Hans">封闭 Alpha 访问密钥</span><input type="password" name="login_key" minlength="32" autocomplete="off" required></label><button type="submit">Enter Paper Raid / <span lang="zh-Hans">进入论文远征</span></button><output></output></form></section>"#;
...
pub(crate) fn practice(
    ...
    let progress = role_steps
        ...
        .map(|(index, (role, objective))| {
            ...
            format!(
                r#"<li data-practice-step-state="{}"{} aria-labelledby="practice-step-meta-{} practice-step-objective-{} practice-step-status-{}"><span class="practice-step-meta" id="practice-step-meta-{}">{} · {}</span><strong id="practice-step-objective-{}">{}</strong><span class="practice-step-status" id="practice-step-status-{}">{}</span></li>"#,
                status,
                aria_current,
                step,
                step,
...
fn page(title: &str, player: &str, content: &str, authenticated: bool) -> Response {
    let key_vault = if authenticated {
        r#"<section class="panel key-vault"><details><summary>Local signing key / <span lang="zh-Hans">本地签名密钥</span></summary><p class="muted">Import your encrypted recovery bundle when this tab needs to sign a human action. Decryption stays in memory and is discarded when the tab closes.</p><form class="human-key-import-form"><label>Encrypted key bundle / <span lang="zh-Hans">加密密钥包</span><input name="key_bundle" type="file" accept="application/json,.json" required></label><label>Passphrase / <span lang="zh-Hans">口令</span><input name="passphrase" type="password" minlength="16" autocomplete="current-password" required></label><button type="submit" disabled>Decrypt into this tab / <span lang="zh-Hans">仅解密到当前标签页</span></button><output></output></form><button class="forget-human-key" type="button">Forget in-memory key / <span lang="zh-Hans">清除内存密钥</span></button><output class="human-key-status">No in-memory key / <span lang="zh-Hans">当前无内存密钥</span></output></details></section>"#
    } else {
        ""
    };
    let document = format!(
...
const CSS: &str = r#"
:root{color-scheme:dark;--bg:#070b12;--panel:#101826;--line:#5f7da6;--text:#e9f2ff;--muted:#8ba0ba;--cyan:#55e6ff;--amber:#ffca68;--pink:#ff6ba8;--focus-backdrop:#070b12}
...
mod tests {
    ...
    async fn practice_renders_one_bounded_player_action_for_each_browser_stage() {
        for (stage, step, action, choices) in [
            ...
            assert!(body.contains(">Current / <span lang=\"zh-Hans\">当前</span></span>"));
            if step > 1 {
                assert!(body.contains(">Completed / <span lang=\"zh-Hans\">已完成</span></span>"));
            }
            if step < 5 {
                assert!(body.contains(">Upcoming / <span lang=\"zh-Hans\">即将开始</span></span>"));
            }
    ...
    async fn practice_waits_for_signed_agent_and_completion_marks_all_steps_done() {
        ...
        assert_eq!(
            completed_body
                .matches(">Completed / <span lang=\"zh-Hans\">已完成</span></span>")
                .count(),
            5
        );
