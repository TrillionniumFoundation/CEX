...
world_map_result = send_and_wait('/map', 'trillionnium_world_map', lambda content, card: int(card.get('node_count') or 0) >= 8 and bool(card.get('current_node_id')) and card.get('has_real_world_map_engine') is True and card.get('map_engine_id') == 'leaflet_openstreetmap_v1' and card.get('tile_provider') == 'OpenStreetMap' and card.get('mirror_scope') == 'global_real_world_tiles' and map_renderer_adapter_ok(card) and bool(card.get('active_region_id')) and ('route_preview_item_count' in card) and ('route_task_graph_count' in card) and route_story_ok(card, command_prefix='/world action') and route_opportunity_target_ok(card, expected_action_label='Open world action lane') and route_runner_handoff_ok(card) and card.get('has_jianghu_combat_log') is True and card.get('jianghu_combat_log_contract') == 'trillionnium_jianghu_combat_log_v1' and int(card.get('jianghu_combat_log_beat_count') or 0) >= 1 and card.get('jianghu_osm_objective_contract') == 'trillionnium_jianghu_osm_objective_v1' and int(card.get('jianghu_osm_objective_count') or 0) >= 1 and card.get('jianghu_npc_relationship_contract') == 'trillionnium_jianghu_npc_relationship_v1' and card.get('tactics_combat_resolution_contract') == 'trillionnium_tactics_combat_resolution_v1' and card.get('tactics_game_session_contract') == 'trillionnium_tactics_game_session_v1' and card.get('tactics_simulation_tick_contract') == 'trillionnium_tactics_simulation_tick_v1' and card.get('map_overlay_identity_contract') == 'trillionnium_map_overlay_identity_v1' and int(card.get('map_overlay_identity_count') or 0) >= 1 and '江湖战报' in str(content.get('body') or ''))
...
print(json.dumps({
    ...
    'world_map_jianghu_combat_log_contract': world_map_result['reply']['card'].get('jianghu_combat_log_contract'),
    'world_map_jianghu_combat_log_beat_count': world_map_result['reply']['card'].get('jianghu_combat_log_beat_count'),
    'world_map_jianghu_osm_objective_contract': world_map_result['reply']['card'].get('jianghu_osm_objective_contract'),
    'world_map_jianghu_osm_objective_count': world_map_result['reply']['card'].get('jianghu_osm_objective_count'),
    'world_map_jianghu_npc_relationship_contract': world_map_result['reply']['card'].get('jianghu_npc_relationship_contract'),
