...
def main():
    ...
    if not os.path.isabs(requested_repo) or os.path.normpath(requested_repo) != repo:
        fail("Hepta source verifier requires a canonical absolute repository path")
    if not re.fullmatch(rb"(?:[0-9a-f]{40}|[0-9a-f]{64})", expected_revision):
        fail("Hepta source verifier revision is not canonical")
    if (
        not re.fullmatch(rb"(?:[0-9a-f]{40}|[0-9a-f]{64})", expected_tree)
        or len(expected_tree) != len(expected_revision)
    ):
        fail("Hepta source verifier tree is not canonical")
    git_path = shutil.which("git", path=os.defpath)
    if git_path is None:
        fail("Hepta source verifier cannot find the system Git binary")
    ...
    def git_output(*arguments):
        ...
        if result.returncode != 0:
            fail("Hepta source verifier could not verify Git authority")
    ...
    def nul_records(payload, authority):
        ...
        if not payload.endswith(b"\0"):
            fail(f"Hepta source verifier received malformed {authority}")
    ...
    def verify_identity_and_status():
        ...
        if observed_root != repo:
            fail("Hepta source verifier repository identity changed")
        if observed_revision != expected_revision:
            fail("Hepta source verifier HEAD changed")
        if observed_tree != expected_tree:
            fail("Hepta source verifier tree changed")
        if git_output(
            b"status",
            b"--porcelain=v1",
            b"-z",
            ...
            fail("Hepta source verifier requires a clean worktree")
    ...
    for record in nul_records(
        ...
        if len(fields) != 3:
            fail("Hepta source verifier received malformed commit tree metadata")
        mode, object_type, object_id = fields
        if object_type != b"blob" or mode not in {b"100644", b"100755"}:
            fail(f"Hepta source verifier rejects tracked entry: {display_path(path)}")
        if path in commit_entries:
            fail("Hepta source verifier found a duplicate commit-tree path")
    ...
    for record in nul_records(
        ...
        fields = metadata.split(b" ")
        if len(fields) != 3:
            fail("Hepta source verifier received malformed index metadata")
        mode, object_id, stage = fields
        if stage != b"0" or mode not in {b"100644", b"100755"}:
            fail(f"Hepta source verifier rejects index entry: {display_path(path)}")
        if path in index_entries:
            fail("Hepta source verifier found a duplicate index path")
        if commit_entries.get(path) != (mode, object_id):
            fail(f"Hepta source verifier index differs from HEAD: {display_path(path)}")
        index_entries[path] = (mode, object_id)
    if index_entries != commit_entries:
        fail("Hepta source verifier index path set differs from HEAD")
    ...
    for record in nul_records(
        ...
        if len(record) < 3 or record[1:2] != b" ":
            fail("Hepta source verifier received malformed index flags")
        flag = record[:1]
        path = record[2:]
        if flag != b"H":
            fail(
                "Hepta source verifier rejects assume-unchanged, skip-worktree, "
                f"or non-ordinary index entry: {display_path(path)}"
            )
        if path in index_flag_paths:
            fail("Hepta source verifier found duplicate index flag metadata")
        index_flag_paths.add(path)
    if index_flag_paths != set(index_entries):
        fail("Hepta source verifier index flag path set differs from HEAD")
    ...
    for authority_path in (
        ...
        if authority_path not in commit_entries and os.path.lexists(
            ...
            fail(
                "Hepta source verifier rejects an untracked toolchain authority: "
    ...
    for path, (expected_mode, expected_object_id) in sorted(index_entries.items()):
        if (
            ...
            fail("Hepta source verifier rejects a non-canonical tracked path")
        worktree_path = os.path.join(repo, path)
        if os.path.realpath(worktree_path) != worktree_path:
            fail(
                "Hepta source verifier rejects a symlinked tracked path component: "
                f"{display_path(path)}"
            )
        try:
            before = os.stat(worktree_path, follow_symlinks=False)
        except OSError:
            fail(f"Hepta source verifier cannot stat: {display_path(path)}")
        if not stat.S_ISREG(before.st_mode):
            fail(f"Hepta source verifier tracked path is not regular: {display_path(path)}")
        if before.st_uid != os.geteuid() or before.st_nlink != 1:
            fail(
                "Hepta source verifier tracked path owner/link count is unsafe: "
                f"{display_path(path)}"
            )
        expected_executable = expected_mode == b"100755"
        actual_executable = bool(stat.S_IMODE(before.st_mode) & 0o111)
        if actual_executable != expected_executable:
            fail(f"Hepta source verifier mode differs from HEAD: {display_path(path)}")
        actual_object_id = git_output(
            b"hash-object", b"--no-filters", b"--", path
        ).strip()
        if actual_object_id != expected_object_id:
            fail(f"Hepta source verifier bytes differ from HEAD: {display_path(path)}")
        try:
            after = os.stat(worktree_path, follow_symlinks=False)
        except OSError:
            fail(f"Hepta source verifier cannot restat: {display_path(path)}")
        before_identity = (
            before.st_dev,
            before.st_ino,
            before.st_mode,
        ...
        if after_identity != before_identity:
            fail(f"Hepta source verifier path changed while hashing: {display_path(path)}")
