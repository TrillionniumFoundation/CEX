...
FLOATING = re.compile(
    r"(?i)(?:toolchain|rust(?:c)?(?:[-_ ]version)?)[^\n]{0,48}(?:stable|latest)"
)
problems: list[str] = []
scanned: list[str] = []
...
result = {
    "schema": "cex.rust-toolchain-convergence-check.v2",
    "status": "failed" if problems else "ok",
    "expected_rustc_toolchain": EXPECTED,
    "cargo_version_policy": "record_exact_executed_identity_without_assuming_point-version_equality",
