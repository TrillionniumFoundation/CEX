...
"$CEX_PROJECT_ROOT/scripts/check-trnm-economy-resource-budgets.sh" >/dev/null
...
for _ in $(seq 1 60); do
  if curl -fsS http://127.0.0.1:7002/v1/trnm/economy/readiness \
      | jq -e '.status == "ok" and .postgres_operations_healthy == true and
        .postgres_operations.pool_saturation_healthy == true and
        .postgres_operations.pool_max_connections == 8' >/dev/null 2>&1 \
    && curl -fsS http://127.0.0.1:8090/health >/dev/null 2>&1; then
    break
  fi
  sleep 1
done
curl -fsS http://127.0.0.1:7002/v1/trnm/economy/readiness \
  | jq -e '.status == "ok" and .postgres_operations_healthy == true and
    .postgres_operations.pool_saturation_healthy == true and
    .postgres_operations.pool_max_connections == 8' >/dev/null
