...
def secondary_freeze_markers() -> list[str]:
    ...
    for pattern in SECONDARY_FREEZE_PATTERNS:
        markers.update(
            ...
            # Any matching path is a secondary authority, including a
            # directory, FIFO, or symlink.  Filtering with ``is_file`` would
            # follow a symlink and let a non-regular marker evade the guard.
...
def main() -> int:
    ...
    if trigger.is_symlink() or not trigger.is_file():
    ...
    result["candidate_trigger_authority"] = {
        "path": TRIGGER_PATH,
        "sole_authority": trigger.is_file() and not trigger.is_symlink() and not secondary_markers,
