...
assert status == 200, status
app_session_status, app_session_html = get('/app', headers=cookie_header)
assert app_session_status == 200, app_session_status
for needle in ['app-account-session-card', 'trillionnium_game_account_surface_session_v1', 'data-session-active="true"', 'data-auth-state="upstream_signed_session"', matrix_user_id, room_id, '/account?return_to=/app']:
...
for needle in ['app-account-session-card', 'trillionnium_game_account_surface_session_v1', 'data-session-active="false"', 'data-auth-state="signed_session_required"', '/account?return_to=/app']:
    assert needle in app_html, ('app_account_session_bridge', needle)
app_api_status, app_api_body = get('/v1/client/app/%40alice%3Alocal.dev')
assert app_api_status == 200, app_api_status
app_api_json = json.loads(app_api_body)
...
normalized_read_switch_enabled = os.environ.get('CONSUMER_ENTRY_LEAGUE_NORMALIZED_READ_SWITCH_ENABLED', '').lower() == 'true'
if normalized_read_switch_enabled:
    receipt_read_model = feed_json.get('normalized_receipt_read_model') or {}
    receipt_snapshot = (feed_json.get('snapshots') or {}).get('term_exchange_receipts') or {}
    receipt_projection = feed_json.get('term_exchange_receipt_projection') or {}
    receipt_items = [item for item in (feed_json.get('items') or []) if item.get('feed_kind') == 'term_exchange_receipt']
    assert receipt_read_model.get('source') == 'normalized_sql_client_feed_read_model', receipt_read_model
    assert receipt_snapshot.get('count', 0) >= 1, receipt_snapshot
    assert len(receipt_snapshot.get('recent') or []) >= 1, receipt_snapshot
    assert receipt_projection.get('contract_version') == 'trillionnium_term_exchange_receipt_projection_v1', receipt_projection
    assert receipt_projection.get('runtime_read_model_source') == 'normalized_sql_client_feed_read_model', receipt_projection
    assert len(receipt_items) >= 1, receipt_items
    app_receipt_read_model = app_api_json.get('normalized_receipt_read_model') or {}
    app_feed = app_api_json.get('feed') or {}
    app_feed_read_model = app_feed.get('normalized_receipt_read_model') or {}
    app_feed_projection = app_feed.get('term_exchange_receipt_projection') or {}
    app_feed_items = [item for item in (app_feed.get('items') or []) if item.get('feed_kind') == 'term_exchange_receipt']
    app_feed_item_count = int(app_feed.get('item_count') or 0)
    app_coach_feed_count = int((((app_api_json.get('playability_coach') or {}).get('context') or {}).get('feed_item_count')) or 0)
    app_ops_feed_count = int((((app_api_json.get('economy_retention_ops') or {}).get('live_counts') or {}).get('feed_item_count')) or 0)
    assert app_receipt_read_model.get('source') == 'normalized_sql_client_app_feed_overlay', app_receipt_read_model
    assert app_feed_read_model.get('source') == 'normalized_sql_client_feed_read_model', app_feed_read_model
    assert app_feed_projection.get('runtime_read_model_source') == 'normalized_sql_client_feed_read_model', app_feed_projection
    assert len(app_feed_items) >= 1, app_feed_items
    assert app_feed_item_count >= len(app_feed_items), app_feed_item_count
    assert app_coach_feed_count == app_feed_item_count, (app_coach_feed_count, app_feed_item_count)
    assert app_ops_feed_count == app_feed_item_count, (app_ops_feed_count, app_feed_item_count)
    assert 'normalized_sql_client_app_feed_overlay' in app_html, 'static /app shell missing client-app normalized receipt overlay marker'
    assert 'normalized_sql_client_feed_read_model' in app_html, 'static /app shell missing embedded feed normalized receipt read-model marker'
...
summary = {
    ...
    'has_client_app_feed_normalized_receipt_read_model': (feed_json.get('normalized_receipt_read_model') or {}).get('source') == 'normalized_sql_client_feed_read_model',
    'has_client_app_api_normalized_receipt_read_model': (app_api_json.get('normalized_receipt_read_model') or {}).get('source') == 'normalized_sql_client_app_feed_overlay',
    'has_client_app_static_normalized_receipt_read_model': 'normalized_sql_client_app_feed_overlay' in app_html and 'normalized_sql_client_feed_read_model' in app_html,
    'client_app_api_feed_normalized_receipt_read_model_source': ((app_api_json.get('feed') or {}).get('normalized_receipt_read_model') or {}).get('source'),
    'client_app_api_feed_term_exchange_receipt_projection_source': ((app_api_json.get('feed') or {}).get('term_exchange_receipt_projection') or {}).get('runtime_read_model_source'),
    'client_app_api_feed_term_exchange_receipt_item_count': len([item for item in ((app_api_json.get('feed') or {}).get('items') or []) if item.get('feed_kind') == 'term_exchange_receipt']),
    'client_app_api_feed_item_count': (app_api_json.get('feed') or {}).get('item_count'),
    'client_app_api_playability_feed_item_count': (((app_api_json.get('playability_coach') or {}).get('context') or {}).get('feed_item_count')),
    'client_app_feed_term_exchange_receipt_projection_source': (feed_json.get('term_exchange_receipt_projection') or {}).get('runtime_read_model_source'),
    'client_app_feed_term_exchange_receipt_item_count': len([item for item in (feed_json.get('items') or []) if item.get('feed_kind') == 'term_exchange_receipt']),
    ...
    'has_client_app_account_session_bridge': all(token in app_html for token in ['app-account-session-card', 'trillionnium_game_account_surface_session_v1', 'data-session-active="false"', 'data-auth-state="signed_session_required"', '/account?return_to=/app']),
    'has_signed_account_session_bridges': all(token in app_session_html for token in ['app-account-session-card', 'data-session-active="true"', 'data-auth-state="upstream_signed_session"', matrix_user_id, room_id, '/account?return_to=/app']) and all(token in world_session_html for token in ['world-account-session-card', 'data-session-active="true"', 'data-auth-state="upstream_signed_session"', matrix_user_id, room_id, '/account?return_to=/world']),
    ...
    'has_world_account_session_bridge': all(token in world_html for token in ['world-account-session-card', 'trillionnium_game_account_surface_session_v1', 'data-session-active="false"', '/account?return_to=/world']),
    ...
    'has_world_focus_action_rail': 'world-map-focus-summary' in world_html_after and 'world-map-action-rail' in world_html_after and '地图行动栏' in world_html_after and 'trillionnium_world_rust_map_focus_ui_fragments_v1' in world_html_after and 'rust_owned_map_focus_ui_fragments' in world_html_after,
