...
def utc_datetime(value: Any, path: str) -> str:
    ...
    require(raw.endswith("Z"), f"{path} must use UTC Z form")
    try:
        parsed = datetime.fromisoformat(raw.replace("Z", "+00:00"))
    except ValueError as error:
        raise ContractError(f"{path} must be RFC3339/date-time: {error}") from error
    require(parsed.utcoffset() == timezone.utc.utcoffset(parsed), f"{path} must be UTC")
    return raw
...
def validate_manifest(data: Any) -> None:
    ...
    require(
        commit_sha != "0" * 40 and tree_sha != "0" * 40,
        "source identity must not be a placeholder",
    )
    ...
    require(
        database.get("migration_head")
        == "0087_add_term_exchange_receipt_event_history.sql",
        "migration head is stale",
    )
    ...
    require(
        ...
        "workflow_run_id is invalid",
    )
    artifacts = build.get("artifacts")
    require(
        isinstance(artifacts, list) and len(artifacts) == 1,
        "candidate requires exactly one evidence payload",
    )
    payload = object_at(artifacts[0], "$.build.artifacts[0]")
    payload_name = nonempty_string(payload.get("name"), "$.build.artifacts[0].name")
    require(
        payload_name.startswith(f"cex-p0-evidence-{commit_sha}-attempt-"),
        "payload name is not bound to commit/attempt",
    )
    ...
    require(
        build.get("images") == [],
        "candidate build images must be empty until separately qualified",
    )
    ...
    for index, raw in enumerate(evidence):
        ...
        require(
            ...
            f"evidence {name} must be pass; waivers are forbidden",
        )
        require(item.get("waiver") is None, f"evidence {name} must not carry a waiver")
        immutable_uri(
            item.get("uri"),
            f"$.evidence[{index}].uri",
            hosted=name.startswith("hosted:"),
        )
        canonical_sha256(item.get("sha256"), f"$.evidence[{index}].sha256")
    require(
        names == list(EXPECTED_EVIDENCE),
        "candidate evidence must be the canonical ordered thirteen-record contract",
    )
    require(
    ...
    require(
        ...
        "candidate requires exactly one repository automation approval",
    )
    approval = object_at(approvals[0], "$.approvals[0]")
    require(
        approval.get("role") == "repository-qualification-automation",
        "approval role is invalid",
    )
    ...
    require(
        approval.get("scope") == APPROVAL_SCOPE,
        "approval scope exceeds repository qualification",
    )
    utc_datetime(approval.get("decided_at"), "$.approvals[0].decided_at")

    external = object_at(root.get("external_gates"), "$.external_gates")
    require(
        external.get("status") == "independent_approval_required",
        "external gate status is invalid",
    )
    require(
        external.get("items") == EXPECTED_EXTERNAL_GATES,
        "external X1-X8 gate set/order is invalid",
    )
